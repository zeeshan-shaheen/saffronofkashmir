...
- Cloudflare Managed robots.txt disabled, 10 Aug 2026. GPTBot,
  ...
  read the site. Live robots.txt is the repo file only, a static
  file in the repo root, hand-maintained, not generated by build.js.
- Content-Signal: search=yes,ai-input=yes,ai-train=no added to
  robots.txt, 10 Aug 2026, with an explanatory preamble. AI
  crawlers can read the site; training use is expressly reserved.
  Declaration only, not enforced.
